...
<body>
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="redirect.php" class="nav-item">
        ...
        <main class="main-content">
            ...
            <div class="table-section">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <h2 class="section-title" style="margin-bottom: 0;">公告列表</h2>
                    <button class="btn btn-primary" onclick="openModal('noticeModal')">添加公告</button>
                </div>

                <?php if (empty($notices)): ?>
                <div class="empty-state">暂无公告，点击上方按钮添加</div>
                <?php else: ?>
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo e(mb_substr($notice['content'] ?? '', 0, 50)) . (mb_strlen($notice['content'] ?? '') > 50 ? '...' : ''); ?></td>
                            <td><?php echo $notice['sort_order']; ?></td>
                            <td>
                                <label class="toggle-switch">
                                    <input type="checkbox" <?php echo $notice['is_active'] ? 'checked' : ''; ?> onchange="toggleStatus('notice', <?php echo $notice['id']; ?>, this.checked)">
    ...
    <div class="modal-overlay" id="noticeModal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="noticeModalTitle">添加公告</h2>
                <button class="modal-close" onclick="closeModal('noticeModal')">&times;</button>
            </div>
            <form id="noticeForm" onsubmit="return saveNotice(event)">
                <input type="hidden" id="noticeId" name="id" value="0">
                <div class="form-group">
                    <label>公告标题</label>
                    <input type="text" id="noticeTitle" name="title" placeholder="请输入公告标题" required>
                </div>
                <div class="form-group">
                    <label>公告内容</label>
                    <textarea id="noticeContent" name="content" placeholder="请输入公告内容"></textarea>
                </div>
                <div class="form-group">
                    <label>排序</label>
                    <input type="number" id="noticeSort" name="sort_order" value="0" min="0">
                </div>
                <div class="form-group">
                    <label>
                        <input type="checkbox" id="noticeActive" name="is_active" value="1" checked>
                        启用
                    </label>
                </div>
    ...
    <script>
        async function saveNotice(e) {
            ...
            data.is_active = document.getElementById('noticeActive').checked ? 1 : 0;

            saveData('notice', data, () => {
        ...
        function editNotice(btn) {
            ...
            document.getElementById('noticeSort').value = sortOrder;
            document.getElementById('noticeActive').checked = isActive === 1;
